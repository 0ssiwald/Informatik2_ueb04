...
void loadPlayer(FILE *File, char *Row)
{
   ...
   current_player->GoalNumber = 0;  //setting default values if the filde does not contain these infos
   ...
   do
   {
      ...
      if(!strncmp(Row_begin, "<Goals>", 7))
      {
         ...
         if(!strncmp(Row_begin + 7 + len, "</Goals>", 8))
            current_player->ShirtNumber = atoi(Row_begin + 7);
      }
      if(feof(File)) //to test of end of file
      break;
